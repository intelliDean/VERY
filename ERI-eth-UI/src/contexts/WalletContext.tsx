...
export const WalletProvider: React.FC<WalletProviderProps> = ({ children }) => {
  ...
  const [ownershipRContract, setOwnershipRContract] =
    useState<ethers.Contract | null>(null);
  const [ownershipSContract, setOwnershipSContract] =
    useState<ethers.Contract | null>(null);
  const [authenticityRContract, setAuthenticityRContract] =
    useState<ethers.Contract | null>(null);
  const [authenticitySContract, setAuthenticitySContract] =
    useState<ethers.Contract | null>(null);

  const [isUserRegistered, setIsUserRegistered] = useState(false);
  const [userRegisteredName, setUserRegisteredName] = useState("");
  const [isManufacturerRegistered, setIsManufacturerRegistered] =
    useState(false);
  const [manufacturerRegisteredName, setManufacturerRegisteredName] =
    useState("");
  ...
  const initializeProvider = async () => {
    if (typeof window.ethereum !== "undefined") {
      ...
      setOwnershipRContract(
        new ethers.Contract(OWNERSHIP_ADDRESS, OWNERSHIP_ABI, web3Provider)
      );
      setAuthenticityRContract(
        new ethers.Contract(
  ...
  const connectExistingWallet = async (
    ...
    try {
      ...
      setOwnershipSContract(
        new ethers.Contract(OWNERSHIP_ADDRESS, OWNERSHIP_ABI, signerInstance)
      );
      setAuthenticitySContract(
        new ethers.Contract(
          AUTHENTICITY_ADDRESS,
          AUTHENTICITY_ABI,
          signerInstance
  ...
  const disconnectWallet = () => {
    ...
    if (provider) {
      ...
      setOwnershipRContract(
        new ethers.Contract(OWNERSHIP_ADDRESS, OWNERSHIP_ABI, provider)
      );
      setAuthenticityRContract(
        new ethers.Contract(AUTHENTICITY_ADDRESS, AUTHENTICITY_ABI, provider)
      );
    }
  ...
  const checkManufacturerRegistration = async (address?: string) => {
    ...
    try {
      const manufacturer = await authenticityRContract.getManufacturer(
        targetAddress
      );
